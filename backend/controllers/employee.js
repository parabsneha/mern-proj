...
// exports.editTaskStatus = (req, res, next) =>{  //working fine 
//   const _id = req.user._id; 
//   const updatedStatus = req.body.status;

//   Employee.findById(req.session.user._id)
//   .then(employee=>{
//     return employee.editTaskStatus(_id, updatedStatus);
//   })
//   .catch(err => console.log(err));
...
//         const taskDate = moment(employeeTask.task[i].date).format("YYYY-MM-DD");
//         const dateArray = taskDate.split('-');
//         const taskMonth = Number(dateArray[1]);
//         const newdate = employeeTask.task[i].date.toLocaleDateString();
//         if (newdate == date && taskMonth == new Date().getMonth() + 1) {
//           console.log("task date inside if ", newdate);
//           taskDetails.push({
...
//         res.json({ message: "sorry! nothing to fetch " });
//       }
//       else {
